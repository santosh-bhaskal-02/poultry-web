...
const recalcDerivedValues = (
  ...
  const netCostPerKg = totalBirdsWeight > 0 ? grossProductionCost / totalBirdsWeight : 0; // Gross Production cost / Total Birds Weight
  setFieldValue("netCostPerKg", Number(netCostPerKg.toFixed(2)));

  const stdCostPerKg = numValue(values.stdCostPerKg); // fixed / user-supplied (default 80)

  // 5) Rearing charges
  const productionCostIncentive = (stdCostPerKg - netCostPerKg) / 2; // (Std. Cost / Kg - Net.Cost/kg) / 2
  setFieldValue("productionCostIncentive", Number(productionCostIncentive.toFixed(2)));

  const rearingChargesStd = productionCostIncentive + 6.5; // ((Std - Net)/2) + 6.50
  setFieldValue("rearingChargesStd", Number(rearingChargesStd.toFixed(2)));

  const rearingChargesPerBird = rearingChargesStd * avgBodyWeight; // incentive * Average Weight
  setFieldValue("rearingChargesPerBird", Number(rearingChargesPerBird.toFixed(2)));

  const totalRearingCharges = rearingChargesStd * totalBirdsWeight; // incentive * Total Birds Sold
  setFieldValue("totalRearingCharges", Number(totalRearingCharges.toFixed(2)));

  // 6) Final bill
  setFieldValue("totalAmountPayable", Number(totalRearingCharges.toFixed(2))); // Total Rearing Charges
};
...
const BirdInventory = () => {
  ...
  const initialValues: FinalReportFormValues = {
    ...
    medicineCost: "",
    adminCost: "",
    grossProductionCost: 0,
    netCostPerKg: 0,
    stdCostPerKg: 80, // Std. cost / Kg (fixed 80.00)

    rearingChargesStd: 0,
    productionCostIncentive: 0,
    rearingChargesPerBird: 0,
    totalRearingCharges: 0,
          ...
          <div className="bg-white p-5 rounded-xl shadow-md border border-emerald-100 w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <InputField
                ...
                onChange={(e) =>
                  recalcDerivedValues("totalBirds", e.target.value, values, setFieldValue)
                }
          ...
          <div className="bg-white p-5 rounded-xl shadow-md border border-emerald-100 w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              <InputField
                ...
                onChange={(e) =>
                  recalcDerivedValues("totalBags", e.target.value, values, setFieldValue)
                }
                icon={<BoxIcon className="w-4 h-4 text-emerald-600" />}
              />
          ...
          <div className="bg-white p-5 rounded-xl shadow-md border border-emerald-100 w-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <InputField
                ...
                onChange={(e) =>
                  recalcDerivedValues("adminCost", e.target.value, values, setFieldValue)
                }
              />
